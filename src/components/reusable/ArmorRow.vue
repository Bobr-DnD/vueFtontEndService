...
function addRow() {
    const armor_one = props.armors_all.filter(w => w.id === event.target.value)[0]
...
<template>
    <div v-for="armor, index in armors"
        class="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_30px] grid-rows-2 p-2 gap-2 items-center justify-items-center font-gothic
        ...
        <div @click.stop="deleteRow(index)"
            class="row-span-2 p-2 w-full bg-darkred-red rounded-xl border-2 border-darkred-dark text-darkred-light font-medium hover:cursor-pointer select-none">
            X</div>
    ...
    <select name="Armors" id="Armors" @change="addRow" :class="['min-w-fit w-4/5 my-2 px-4 py-2 bg-darkred-light border border-darkred-dark rounded-md text-darkred-dark font-gothic',
